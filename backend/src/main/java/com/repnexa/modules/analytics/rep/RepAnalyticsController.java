...
public class RepAnalyticsController {
  ...
  public RepDetailsResponse repDetails(@RequestBody RepDetailsRequest req, Authentication auth) {
    ...
    if (routeIds == null || routeIds.isEmpty()) {
      return new RepDetailsResponse(List.of(), new Flags(false));
    }

    DateRange range = DateRange.from(req, clock);
    ...
    if (req.repUserId() != null) {
      params.put("repUserId", req.repUserId());
    }
    ...
    String sql =
        ...
        select
          ...
          coalesce((
            ...
            where me.rep_user_id = c.rep_user_id
              and me.route_id in (:routeIds)
              and me.entry_date >= :dateFrom
              and me.entry_date <= :dateTo
          ), 0) as total_mileage_km
        from doctor_calls c
        ...
        jdbc.query(
            ...
            (rs, i) ->
                new RepRow(
                    ...
                    rs.getBigDecimal("total_mileage_km")));
  ...
  public record RepRow(
      long repUserId,
      String repName,
      long visitCount,
      long uniqueDoctors,
      LocalDate lastVisitDate,
      java.math.BigDecimal totalMileageKm) {}
  ...
  record DateRange(LocalDate dateFrom, LocalDate dateTo) {
    static DateRange from(RepDetailsRequest req, Clock clock) {
      LocalDate today = LocalDate.now(clock);
      Period p = req.period() == null ? Period.THIS_MONTH : req.period();
        ...
        case CUSTOM -> {
          if (req.dateFrom() == null || req.dateTo() == null) {
            throw new ApiException(
                400, "VALIDATION_ERROR", "dateFrom and dateTo are required for CUSTOM period");
          }
          if (req.dateTo().isBefore(req.dateFrom())) {
            throw new ApiException(
                400, "VALIDATION_ERROR", "dateTo must be on or after dateFrom");
          }
          yield new DateRange(req.dateFrom(), req.dateTo());
        }
      };
    }
  }
