...
public class AdminDoctorService {
    ...
    public DoctorDtos.DoctorResponse patch(long id, DoctorDtos.PatchDoctorRequest req) {
        ...
        if (d.getDeletedAt() != null) throw ApiException.conflict("DOCTOR_DELETED", "Doctor is deleted");

        if (req != null) {
            if (!isBlank(req.name())) d.setName(req.name().trim());
            if (req.specialty() != null) d.setSpecialty(trimToNull(req.specialty()));
            if (Boolean.TRUE.equals(req.deleted())) d.softDeleteNow();
        }
        Doctor saved = doctors.save(d);
        return new DoctorDtos.DoctorResponse(saved.getId(), saved.getName(), saved.getSpecialty(), saved.getDeletedAt() != null);
    }
